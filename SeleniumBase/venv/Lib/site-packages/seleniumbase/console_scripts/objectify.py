...
Output:
        ...
        "pageObjects.py", supporting the Page Object Pattern.
"""
...
PAGE_OBJECTS_FILE = "pageObjects.py"  # Don't change this. It's hard-coded.
...
def invalid_objectify_run_command():
    ...
    exp += '         into objects stored in a "pageObjects.py" file that is\n'
...
def invalid_inject_objects_run_command():
    ...
    exp += "                         (Default: No added comments.)\n"
    exp += "  Output:\n"
    exp += '         Takes the page objects found in the "pageObjects.py"\n'
...
def invalid_extract_objects_run_command():
    ...
    exp += '         "pageObjects.py" file in the same folder as the tests.\n'
    raise Exception("INVALID RUN COMMAND!\n\n%s" % exp)
...
def invalid_revert_objects_run_command():
    ...
    exp += '         selectors stored in the "pageObjects.py" file.\n'
    raise Exception("INVALID RUN COMMAND!\n\n%s" % exp)
...
def process_test_file(
    ...
    seleniumbase_lines = []
    page_selectors = []
    changed = []  # The classes of pageObjects.py to add to the test import
...
def main(shell_command):
    ...
    if shell_command == "inject-objects" or shell_command == "objectify":
        ...
        for line in seleniumbase_lines:
            if "from .pageObjects import" in line:
                token = line.split("from .pageObjects import ")[1].strip()
                if token in added_classes:
                    # Don't import pageObjects classes if already imported
            ...
            for line in seleniumbase_lines:
                if line.startswith("from") and "import" in line and not fit_in:
                    ...
                    for add_me in added_classes:
                        import_line = "from .pageObjects import %s" % add_me
